...
class PlanarQuadrotor:
    ...
    def optimal_control(self, state, grad_value):
        ...
        possible_vals = [(self.min_thrust_per_prop, self.min_thrust_per_prop), (self.min_thrust_per_prop, self.max_thrust_per_prop), (self.max_thrust_per_prop, self.min_thrust_per_prop), (self.max_thrust_per_prop, self.max_thrust_per_prop)]
        for T1, T2 in possible_vals:
            dyn = self.dynamics(state, [T1, T2])
            H = grad_value.T @ dyn
            #jax.debug.print("Value inside JIT: {}", H)
            optimal_control = jnp.where(H < min_H, jnp.array([T1, T2]), optimal_control) # flip these
